// Generic XMLHttpRequest observer — the single point where v5's MAIN-world
...
//   game traffic ourselves (TOS-critical, see DESIGN.md §3), so hooking
//   XHR is how v5 "sees" what the game is doing in passive observer mode.
//
...
//                   see 4.8.5 commit history for the mobile-race lesson).
...
//   - Mocks / fakes for tests. Tests in v5's test/ directory use the
//     happy-dom XMLHttpRequest shim directly; this file is production code.
 ...
 * non-enumerable symbol-like property names prefixed with `_oge5_` so
...
const installPatch = () => {
  ...
  /** @type {any} */ (XMLHttpRequest.prototype).open = function patchedOpen(
    ...
    /** @type {any} */ (this)._oge5_url = String(url);
    /** @type {any} */ (this)._oge5_method = method;
  ...
  /** @type {any} */ (XMLHttpRequest.prototype).send = function patchedSend(
    ...
    const url = typeof self._oge5_url === 'string' ? self._oge5_url : '';
    const method = self._oge5_method;
 ...
 *       if (!data) return;
 *       document.dispatchEvent(new CustomEvent('oge5:galaxyScanned', {
 *         detail: { ... },
 *       }));
 *     },
 *   });
