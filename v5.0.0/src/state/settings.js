...
//
// The v5 settings panel reads the whole config in one render pass and
...
// localStorage key under the `oge5_` prefix (e.g. `mobileMode` →
// `oge5_mobileMode`). Three reasons:
//
//   1. AGR integration — the browser-script AGR settings panel is older
//      than v5 and expects each preference under its own key. Bundling
//      everything into one JSON blob would break AGR's view/edit flow.
//   2. DevTools debuggability — one preference per key gives a flat,
//      human-readable view in the Application panel (`oge5_colMinGap = 20`
//      beats `oge5_settings = {"...":20,...}` when you are tracing a bug).
//   3. Partial v4 migration — users coexist v4 (`oge_*`) and v5 (`oge5_*`)
//      during the transition. Per-key storage means v5 can adopt a field
//      at a time without locking the whole Settings object to a single
//      schema version.
//
// # Algorithm
//
 ...
 */
export const SETTINGS_PREFIX = 'oge5_';
