// GitHub Gist sync client — the only place v5 speaks to a remote network.
//
// # Role
//
...
// # v4.9.x → v5 migration
//
// OG-E 4.9.0 already adopted the gzip+base64 payload shape that v5
// uses (schema version 3 — yes, the number is shared). v5 keeps the
// *shape* but intentionally uses a **separate gist**: different
// description ({@link GIST_DESCRIPTION} vs {@link GIST_DESCRIPTION_V2})
// and different filename ({@link GIST_FILENAME} vs
// {@link GIST_FILENAME_V2}). The reason is rollback safety. If a user
// installs v5, runs it for a while, then rolls back to v4, both
// versions must remain able to read their own gist with their own
// data. Sharing the gist would mean a v5 write (which may carry v5-
// specific fields) would confuse v4, and a v4 write would strip
// anything v5-specific v4 doesn't know about.
//
// The one-time migration path (see {@link ensureGistV3}) therefore:
//
//   1. Looks for a v5 gist (description matches {@link GIST_DESCRIPTION}).
//      If found, cache its id and we're done.
//   2. If absent, looks for a v4 gist (description matches
//      {@link GIST_DESCRIPTION_V2}). If found, READ its content, COPY
//      `galaxyScans` + `colonyHistory` into a fresh payload, CREATE a
//      new v5 gist. The v4 gist is **never** modified or deleted —
//      the user can roll back and find their v4 data exactly where
//      they left it.
//   3. If neither exists, create an empty v5 gist.
//
...
// through to creating an empty v5 — the user's v4 data is still safe
// on GitHub, and a later merge round can still reconcile from either
// side once the user fixes whatever broke the read.
//
 ...
 * @property {3} version
 *   Schema version. Pinned to 3 since v4.9.0 — readers reject anything
 *   else via {@link fetchGistData}'s schema guard, which keeps us from
 *   accidentally interpreting a v1/v2 blob as the current shape if the
 *   user ever edits the gist by hand.
 ...
 * Namespaced with the `oge5_` prefix every v5 key shares so v4 state
 * never collides.
 */
export const TOKEN_KEY = 'oge5_gistToken';

/** localStorage key caching the v5 gist id once discovered/created. */
export const GIST_ID_KEY = 'oge5_gistId';

/**
 * localStorage key v4.x wrote its gist id to. We clean this up after
 * a successful v5 migration — not because it hurts to leave it (the
 * key lives alongside its matching token, unread by v5) but because
 * stripping it gives support a cleaner storage dump.
 */
export const LEGACY_GIST_ID_KEY = 'oge_gistId';

/** localStorage key holding the ISO timestamp of the last successful upload. */
export const LAST_UP_KEY = 'oge5_lastSyncAt';

/** localStorage key holding the ISO timestamp of the last successful download. */
export const LAST_DOWN_KEY = 'oge5_lastDownAt';

/** localStorage key holding the last error message, or absent on success. */
export const LAST_ERR_KEY = 'oge5_lastSyncErr';

// ── Gist identity ───────────────────────────────────────────────────

/** Filename under which the v5 gist stores its single compressed data file. */
export const GIST_FILENAME = 'oge5-data.json.gz.b64';

/** Description GitHub shows for the v5 gist; also used as the discovery predicate. */
export const GIST_DESCRIPTION = 'OG-E v5 sync data (compressed) — do not edit manually';

/** v4 (legacy) filename — read-only during v4→v5 migration, never written. */
export const GIST_FILENAME_V2 = 'oge-data.json.gz.b64';

/** v4 (legacy) description — matched during v4→v5 migration discovery. */
...
/**
 * Read the cached v5 gist id from localStorage, or `''` when not yet
...
/**
 * Cache the v5 gist id in localStorage. Normally called by
 ...
 * created v5 gist that raced a read before the write settled).
 *
 * @param {any} gist
...
// ── Gist discovery + creation (with v4→v5 migration) ────────────────

/**
 * Return the id of the v5 gist, discovering or creating one as needed.
 *
 * Four-step resolution:
 *
 *   1. **Cached id** — if {@link GIST_ID_KEY} is populated, return it
 *      without a network call.
 *   2. **Existing v5 gist** — list the user's gists (one page of 100;
 *      enough for any real account), match `description ===
 *      {@link GIST_DESCRIPTION}`. If found, cache and return its id.
 *   3. **v4 migration** — if no v5 gist but a v4 gist
 *      (`description === {@link GIST_DESCRIPTION_V2}`) exists, read and
 *      decode its content. Copy `galaxyScans` + `colonyHistory` into
 *      the initial payload. The v4 gist is **never** modified.
 *   4. **Fresh create** — POST a new v5 gist with the initial payload
 *      (migrated from v4 if step 3 succeeded, empty otherwise). On
 ...
 * creating an empty v5 gist so the user isn't blocked forever on a
 * corrupted legacy payload.
 *
 * @returns {Promise<string>} The v5 gist id.
...
export const ensureGistV3 = async () => {
  ...
  if (v2) {
    try {
      ...
      if (content) {
        ...
        // schema version we understand. A v1/v2-JSON-only blob with a
        // different `version` is skipped; we'd rather create an empty
        // v5 and let the merge engine rehydrate from local state.
      ...
      // Best-effort migration. The v4 gist is still on GitHub for
      // rollback; the user can try again next boot. We don't rethrow
      // because a migration failure mustn't block v5 boot.
  ...
  // Clean up the v4 cache pointer. The v4 gist itself stays on GitHub;
  // we only drop the localStorage reference because v5 never reads it.
...
/**
 * Pull and decode the current payload from the v5 gist. Returns `null`
...
export const fetchGistData = async () => {
  ...
  try {
    const json = await gzipDecode(content);
    const parsed = JSON.parse(json);
    // `parsed.version || 1` preserves the legacy v1 numbering: older
    // payloads that pre-date explicit versioning are rejected, same
    // as anything labelled with a version we don't understand.
 ...
 * v5 gist's data file. Creates / discovers the gist if needed.
 *
 ...
 * The "Clear scan data" UI action in v4 needed a matching remote wipe
 * (merge logic alone can't distinguish "user cleared" from "device
 * hasn't seen this"); v5 preserves the same primitive.
 *
