 ...
 *
 * NOTE: this function does NOT implement the `deletedSet` tombstone
 * filter from 4.x `sync.js`. Tombstones are the caller's concern
 * — they filter both sides BEFORE passing in, or filter the merged
 * result AFTER. Keeping merge tombstone-free is what makes it a pure
 * reconciliation primitive independent of the tombstone store's
 * schema/location.
 *
