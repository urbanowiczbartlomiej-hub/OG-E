...
// Pure logic helpers for the new sendCol rewrite (see SENDCOL_DESIGN.md).
// Imported by the new orchestrator file (`sendCol.js`, Wave 2). Holds
// target-pick algorithms, DOM coord readers, URL builders, and the
// min-gap wait helper.
//
// Purity levels:
...
//
// `parseFleetdispatchUrlCoords` is deliberately NOT ported — the new
// sendCol reads `fleetDispatcher.targetPlanet` directly (§4 of design).
 ...
 *
 * Powers the §4 "current-view priority" rule of SENDCOL_DESIGN.md —
 * when the user has just scanned a system and is still on its galaxy
