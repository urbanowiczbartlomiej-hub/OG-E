...
// Histogram page I/O — Export / Import of the full v5 dataset (JSON),
...
// two tombstone keys the Phase-10 sync scheduler will observe. It never
// hits the network — all downloads go through Blob + ObjectURL, and
// neither `fetch()` nor `XMLHttpRequest` appear anywhere in this file
// (see DESIGN.md §14 anti-pattern #1).
//
// Shape of the exported JSON (see DESIGN.md §7):
//   {
//     version: 1,
//     exportedAt: <ISO>,
 ...
 * scheduler (running in the game-origin content script) will observe
 * the change via `chrome.storage.onChanged` and force an upload cycle.
 * Its value is a Date.now() timestamp — the scheduler only cares that
 * it changed, not about the exact number.
 */
export const SYNC_REQUEST_KEY = 'oge5_syncRequestAt';

/**
 * Tombstone written when the user clicks "Clear remote". Tells the
 * Phase-10 scheduler to wipe the remote gist contents so local state
...
/**
 * Download the full v5 dataset as a pretty-printed JSON file. Missing
 ...
 * {@link SYNC_REQUEST_KEY}; the Phase-10 scheduler observes the change
 * via `chrome.storage.onChanged` and uploads immediately. In Phase 9
 * nobody listens yet — wiring lands in Phase 10.
 *
 * @returns {Promise<void>}
 */
 ...
 * {@link CLEAR_REMOTE_KEY}; the Phase-10 scheduler uses this to wipe
 * the Gist payload without letting local state re-upload the
 * now-deleted scans. Phase-10 wiring, same as {@link triggerSync}.
 *
 * @returns {Promise<void>}
 */
