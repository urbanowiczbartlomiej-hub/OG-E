...
// @see ../../../histogram.js:262 — v4 reference implementation (lines 262–677)
...
export const renderGalaxyMap = (opts) => {
  ...
  // deciding empty-state. In practice the v5 store writer always sets
...
const renderGalaxySection = (args) => {
  ...
  // Mini stats surface the most actionable counts only — mirror 4.x so
  // users who glance at the header get the same quick read they're used
  // to. Other statuses (mine, occupied, …) are already in the global
  // stats row; repeating them here would bloat the header.
  /** @type {string[]} */
  const parts = [];
