...
describe('installSettingsUi — button', () => {
  it('Sync button dispatches oge5:syncForce CustomEvent on click', async () => {
    setupAGR();
    installSettingsUi();
    await flushWaitFor();
    ...
    document.addEventListener('oge5:syncForce', handler);
    ...
    document.removeEventListener('oge5:syncForce', handler);
    expect(received).toContain('oge5:syncForce');
  });
});
...
describe('installSettingsUi — static status', () => {
  it('renders formatSyncStatus output populated from localStorage timestamps', async () => {
    ...
    localStorage.setItem('oge5_lastSyncAt', iso);
    localStorage.setItem('oge5_lastDownAt', iso);
    setupAGR();
    installSettingsUi();
    await flushWaitFor();
  ...
  it('includes error line when oge5_lastSyncErr is set', async () => {
    localStorage.setItem('oge5_lastSyncErr', 'rate limited');
    setupAGR();
    installSettingsUi();
    await flushWaitFor();
