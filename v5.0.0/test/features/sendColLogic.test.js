...
//
// Unit tests for sendColLogic.js — the pure-logic helpers that the new
// sendCol orchestrator (SENDCOL_DESIGN.md, Wave 2) builds on.
//
// Coverage breakdown:
//   - findNextScanSystem — pure target picker (ported from old tests).
//   - findNextColonizeTarget — pure colonize picker (ported).
//   - pickCandidateInView — NEW: current-view priority target picker.
...
describe('getColonizeWaitTime', () => {
  ...
  it('debug diagnostic logs when oge5_debugMinGap=true', () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date('2026-01-01T00:00:00Z'));
    paintDuration('0:01:00');
    localStorage.setItem('oge5_debugMinGap', 'true');
    const now = Date.now();
    registryStore.set([
      {
        coords: '4:40:8',
