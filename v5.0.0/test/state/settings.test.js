...
// preference under `oge5_`), so tests exercise the wire directly: set
...
describe('SETTINGS_PREFIX and SETTINGS_SCHEMA', () => {
  it('exports the v5 namespaced prefix', () => {
    expect(SETTINGS_PREFIX).toBe('oge5_');
  });
...
describe('initSettingsStore — hydration', () => {
  ...
  it('treats an empty string as a legitimate stored value (not missing)', () => {
    ...
    localStorage.setItem('oge5_colPositions', '');
    initSettingsStore();
  ...
  it('falls back to default when a bool value is unparseable', () => {
    ...
    localStorage.setItem('oge5_expeditionBadges', 'garbage');
    initSettingsStore();
  ...
  it('falls back to default when an int value is unparseable', () => {
    ...
    localStorage.setItem('oge5_colMinGap', 'not-a-number');
    initSettingsStore();
  ...
  it('hydrates a mix of fields at once in a single store update', () => {
    localStorage.setItem('oge5_mobileMode', 'true');
    localStorage.setItem('oge5_colMinGap', '30');
    localStorage.setItem('oge5_colPositions', '7,8');
    localStorage.setItem('oge5_gistToken', 'ghp_abc123');
...
describe('initSettingsStore — write-through (per-key diff)', () => {
  ...
  it('writes multiple changed fields in the same set/update call', () => {
    ...
    expect(localStorage.getItem('oge5_colMinGap')).toBe('30');
    expect(localStorage.getItem('oge5_colPositions')).toBe('7');
    expect(localStorage.getItem('oge5_mobileMode')).toBe('true');
  });
  ...
  it('does NOT touch localStorage keys for fields that did not change', () => {
    ...
    localStorage.setItem('oge5_colBtnSize', 'SENTINEL');
    ...
    expect(localStorage.getItem('oge5_colBtnSize')).toBe('SENTINEL');
  });
  ...
  it('coerces values via String(): true → "true", 42 → "42"', () => {
    ...
    expect(localStorage.getItem('oge5_mobileMode')).toBe('true');
    expect(localStorage.getItem('oge5_expeditionBadges')).toBe('false');
    expect(localStorage.getItem('oge5_enterBtnSize')).toBe('42');
    expect(localStorage.getItem('oge5_gistToken')).toBe('abc');
  });
  ...
  it('writes when a field changes back to its default value', () => {
    ...
    localStorage.setItem('oge5_colMinGap', '60');
    initSettingsStore();
    ...
    expect(localStorage.getItem('oge5_colMinGap')).toBe('20');
  });
});
...
describe('initSettingsStore — idempotent', () => {
  ...
  it('a second init does not install a duplicate write-through', () => {
    ...
    expect(localStorage.getItem('oge5_colMinGap')).toBe('50');
  ...
  it('a second init does not re-run hydration (in-memory changes survive)', () => {
    ...
    expect(localStorage.getItem('oge5_colMinGap')).toBe('77');
