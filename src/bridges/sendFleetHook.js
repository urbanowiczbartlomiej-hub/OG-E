...
//   write MUST complete in the same JS tick as the navigation that
//   follows the response. A post-response chrome.storage.set would lose
//   the entry on mobile Firefox when the game's redirect beats our
//   async callback. `safeLS.setJSON` is fully synchronous, so the
//   registry update is visible before any navigation can start —
//   see state/registry.js for the full rationale.
//
 ...
 *   - `load` phase — on success, dispatches `oge:colonizeSent` on
...
export const installSendFleetHook = () => {
  ...
  const unsubSend = observeXHR({
    ...
    handler: ({ xhr, body }) => {
      ...
      // state/registry.js for the full rationale. Gated on
