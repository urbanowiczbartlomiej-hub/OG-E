...
//
// The `chrome.storage.set` API is asynchronous: on mobile Firefox, a
// quick colonize-then-navigate sequence can start the navigation
// before the browser has flushed the storage write, losing the entry
// and letting a duplicate send slip through. `localStorage`, by
// contrast, is fully synchronous — the entry is visible to any
// subsequent read in the same JS tick, before the navigation begins.
// This is the core "mobile race fix" and the reason this module
// exists separately from the chrome.storage-backed settings.
//
// # Why 0 ms debounce?
//
// Every single `registryStore.set(...)` MUST hit localStorage before
// the next tick can schedule navigation. A 1 ms debounce would
// reintroduce the mobile race. Writes are infrequent (one per fleet
// send, plus the occasional prune) so there is no burst to collapse
// anyway. `debounceMs: 0` means `persist` calls `save` inline inside
// the subscribe callback — which is inside the `store.set` stack
...
export const initRegistryStore = () => {
  ...
  disposeFn = persist({
    ...
    // module header. Do NOT raise this without revisiting the mobile
    // colonize-then-navigate race.
    debounceMs: 0,
  });
  return disposeFn;
};
