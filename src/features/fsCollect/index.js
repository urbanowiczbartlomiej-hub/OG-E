...
const refresh = () => {
  ...
  if (microZone) {
    if (onFleetdispatch() && isStep2()) {
      setLabel(microZone, 'Collectors route', urlCoordsLabel() || undefined, 'Send');
    } else if (onFleetdispatch() && urlHasAm()) {
      setLabel(microZone, 'Collectors route', urlCoordsLabel() || undefined, 'Next');
    } else {
      const body = readCurrentBody();
  ...
  if (collectZone) {
    if (urlHasCollect() && isStep2()) {
      setLabel(collectZone, 'Collect All', urlCoordsLabel() || undefined, 'Send');
    } else if (urlHasCollect()) {
      setLabel(collectZone, 'Collect All', urlCoordsLabel() || undefined, 'Next');
    } else {
