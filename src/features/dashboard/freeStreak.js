 ...
 * @param {Map<number, import('../../domain/dangerScore.js').DangerProfile>} [danger]
 *   Per-player danger profiles — joined by occupant id for the D badge.
 * @returns {HTMLElement}
 */
const buildSystemCard = (g, s, scan, pinned, players, linkBase, danger) => {
  ...
  for (let g = 1; g <= galaxies; g++) {
    ...
    for (let s = 1; s <= systems; s++) {
      ...
      for (let p = 1; p <= POS; p++) {
        ...
        const isHi = highlightPlayer != null && pos && pos.player && pos.player.id === highlightPlayer;
 ...
 *   player's planets (Spyglass → map reverse deep-link).
