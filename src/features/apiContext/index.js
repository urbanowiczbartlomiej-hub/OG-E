...
/**
 * Ensure the local API cache is fresh (missing/stale feeds only, or every feed
 * when `force`). The fetch/parse/cache orchestration now lives in
 * `features/shared/apiRefresh` so the dashboard can drive it too; this is the
 * in-game entry point — same-origin, current-universe — kept for the callers and
 * tests that import `refreshCache`.
 * @type {typeof refreshApiCache}
 */
export const refreshCache = refreshApiCache;
