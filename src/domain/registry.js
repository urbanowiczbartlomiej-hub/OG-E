...
// "fleets we just dispatched on mission=colonize" used to coordinate
// the min-gap timer and to hide already-ours coords from the scan UI.
// Three helpers, no I/O: prune expired entries, dedupe a candidate
// against recent identical sends, detect min-gap conflicts.
//
 ...
 *
 * The default tolerance is 2000 ms. Two seconds is wide enough to
 * catch retry bursts and the rare fleet-redirect hook misfire where
 * the same send shows up twice, and narrow enough that a deliberate
 * re-send of the same coord after the first attempt lands is not
 * falsely swallowed.
 *
 ...
 *
 * "Within minGapMs" uses strict `<`: an entry *exactly* `minGapMs`
 * away is NOT a conflict — at the boundary the gap is already "enough"
 * by definition of minGap.
 *
 ...
 * `null` — the safe default of "no conflict", consistent with
 * returning wait=0 when `#durationOneWay` was unparseable.
 *
...
export const findConflict = (
  ...
  for (const r of reg) {
    ...
    // written import, legacy leftovers, migration remnants) have been
    // observed to carry `arrivalAt` as a string. `string <= now` tricks
    // one-sided coercion and `Math.abs('10000' - 10000)` gives 0,
    // which would mask a real min-gap conflict. An explicit cast here
    // makes every downstream comparison numeric and keeps the logic
    // identical for well-formed data.
