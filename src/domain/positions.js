...
//
// `buildGalaxyOrder` uses modular arithmetic that *wraps* galaxies (so
// from galaxy 1 the "previous" galaxy is the last one, not nothing),
// even though OGame itself does not let ships cross that boundary. The
// wrap keeps the priority list uniform and is kept intentionally; see
// the function's own docstring for detail.
//
...
export const parsePositions = (str) => {
  ...
  for (const part of str.split(',')) {
    ...
    if (range) {
      // Reverse ranges ("10-8") are treated as ascending.
    ...
    // segment to be digits; parseInt would accept "8abc" as 8
    // otherwise, which is sloppier than we want.
 ...
 * Wrapping anyway gives a uniform result (always length `maxGalaxy`,
 * always covers every galaxy once), and the ordering still produces a
 * reasonable near-first priority when the user is near an edge.
 *
