...
| **0b** | Data-honesty enablers: parse universe.xml `<moon>` children → **bodies as coverage denominator**; **admit moon + partial/resources-only reports** via a `revealed{}` map read from the `hidden*` flags, and gate `estimateHiddenFleet`/coverage on `revealed.defense` so a partial report never reads absent defence as 0 (§9bis); equal-ts `>=` guard; rescan-map prune; shared `lib/dangerColor.js` (reconcile palettes). | `apiOccupancy.js`, `espionageReport.js`, `threatModel.js`, `state/targets.js`, `lib/dangerColor.js`, `freeStreak.js`+`targets.js` (color call sites) | small (report gate + shape) |
...
| **E1** | **Rich report fields → normalizer** (§9): map `resources`/metal/crystal/deut, `loot`, `counterEspionageChance`, `highscoreEconomy/Research/Lifeforms`, `characterClass`, `buildings/research/lf`; fix the `activity=-1`→NONE enum. Normalizer-only, no store shape, unlocks `lootNow` + civil de-bias. | `domain/espionageReport.js` (+ SpyReport typedef) | no |
| **F** | Routine tracker: **`{latest,history}` migration as read-time projection on BOTH paths** (`latestOf`/`historyOf` + normalizer rewrite + write-side retention gate + migration test); **capture galaxy-view activity** (`parseActivity` in `classifyPosition` + a compact persisted per-body activity ring, watched-only) with **self-induced-activity discount** (§6.6bis); `domain/routine.js`; activity strip (two sources) + coverage rows + weekday pattern (uses the E1 `resTotal`) + collection callout + spy timeline. **Gated on the ToolDev OK.** | NEW `domain/targetReports.js`, `state/targets.js`, `targetsIngest/index.js`, `bridges/galaxyHook.js`+`domain/scans.js` (activity parse), NEW `domain/routine.js`, `dossier.js`, card | **yes (the one migration)** |
| **G** | Scan plan visibility + priority: `domain/scanPriority.js` (danger×staleness×windowBonus) + `staleMs` cadence + FAB walks the order + "why next" strip + `shipAvailability()` pre-flight + `noShips` label fix. **`windowBonus` gated on the ToolDev OK.** | NEW `domain/scanPriority.js`, `sendSpy/pure.js`+`index.js`, `index.js` (strip) | no |
