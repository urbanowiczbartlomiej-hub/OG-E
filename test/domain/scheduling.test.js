...
describe('RESCAN_AFTER', () => {
  ...
  it('does NOT map `abandoned` (uses `abandonedCleanupDeadline` instead)', () => {
...
describe('isSystemStale', () => {
  ...
  it('reports stale when ANY position is past its threshold, even alongside stable ones', () => {
    ...
    // (`abandoned` uses a dynamic 3-AM deadline, so this uses
    // `inactive` for a flat-threshold mixed-slot test.)
    expect(
      isSystemStale(
        {
