...
describe('normalizeAllianceDoc', () => {
  ...
  it('normalises member blocks and drops malformed players/fields', () => {
    ...
    expect(alice.players['7']).toEqual({ name: 'Bob', tag: 'enemy', lastSpySec: 500, bodiesSpied: 2 });
    // Bad tag + non-numeric timestamp stripped, but the entry survives.
...
describe('buildMemberBlock', () => {
  ...
  it('carries name + tag only when known, and keeps unknown watched players as empty entries', () => {
    const block = buildMemberBlock({
      watchedIds: ['7', '8'],
      relationships: { 7: 'enemy' },
      playerNames: { 7: 'Bob' },
      nowSec: 1,
    });
    expect(block.players['7']).toEqual({ name: 'Bob', tag: 'enemy' });
    expect(block.players['8']).toEqual({});
  });
});
...
describe('mergeOwnBlock', () => {
  it('replaces only the own block, keeps every other member verbatim, and does not mutate', () => {
    ...
    doc.members.Alice = { updatedAtSec: 1, players: { 7: { tag: 'enemy' } } };
...
describe('summarizeAllianceIntel', () => {
  it('unions players across members: newest timestamps win, watchers listed alphabetically', () => {
    ...
    doc.members.Zed = {
      updatedAtSec: 10,
      players: { 7: { name: 'OldName', tag: 'neutral', lastSpySec: 100 } },
    };
    doc.members.Alice = {
      updatedAtSec: 20, // fresher block → her name/tag win
      players: { 7: { name: 'NewName', tag: 'enemy', lastSpySec: 50, lastSeenSec: 400 } },
    };
