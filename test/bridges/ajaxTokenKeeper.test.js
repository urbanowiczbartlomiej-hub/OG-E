...
describe('ajaxTokenKeeper — outgoing traffic', () => {
  it('DETECTS a spent token in an outgoing checkTarget without modifying the body', async () => {
    setPageToken(T0);
    installAjaxTokenKeeper();
    await fakeXHR(EVENTBOX_URL, { responseText: rotationResponse(T1) });

    // A later observer sees exactly what goes on the wire — the keeper's
    // substitution ships disabled, so the spent token must survive untouched.
    /** @type {unknown[]} */
    const seenBodies = [];
    observeXHR({
      urlPattern: /action=checkTarget/,
      on: 'send',
      handler: ({ body }) => void seenBodies.push(body),
    });

    const spentBody = `galaxy=1&system=2&position=3&type=1&token=${T0}&union=0`;
    await fakeXHR(CHECK_TARGET_URL, { method: 'POST', body: spentBody });

    expect(seenBodies).toEqual([spentBody]);
