...
//
// Each case captures `oge:colonizeSent` via a per-test listener,
...
/**
 * Register a one-shot listener for `oge:colonizeSent`. Tests call
...
const captureColonizeEvents = () => {
  ...
  document.addEventListener('oge:colonizeSent', listener);
  return {
    events,
    cleanup: () => document.removeEventListener('oge:colonizeSent', listener),
  };
};
...
describe('installSendFleetHook — colonize pre-register (send phase)', () => {
  ...
  it('dispatches oge:colonizeSent after a successful response', async () => {
    installSendFleetHook();
  ...
  it('pre-registers on mission=7 even when response.success is false, but dispatches no event', async () => {
    // Semantics: the registry write happens BEFORE the response
...
describe('installSendFleetHook — duration edge cases', () => {
  it('skips LS write when #durationOneWay is unparseable (textContent = "abc")', async () => {
    // Decision: arrivalAt=0 is never written to localStorage (the
    // guard is `arrivalAt > 0`). We still dispatch the event on
