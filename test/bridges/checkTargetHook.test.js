...
// uses, just one layer higher: here we assert on the `oge:checkTargetResult`
...
describe('installCheckTargetHook — idempotency', () => {
  it('returns the same unsubscribe on repeated install and does not double-dispatch', async () => {
    ...
    document.addEventListener('oge:checkTargetResult', counter);
    ...
    document.removeEventListener('oge:checkTargetResult', counter);
    expect(count).toBe(1);
  });
