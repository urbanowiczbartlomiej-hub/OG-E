...
describe('ensureGistV3', () => {
  ...
  it('caches and returns the gist id when one with the current description already exists', async () => {
    // No cache; gists listing includes a matching gist. We should adopt
    // its id without creating anything new.
    localStorage.removeItem(GIST_ID_KEY);
    const fetchMock = vi.fn().mockResolvedValueOnce(
      makeResponse({
        body: [
          { id: 'current-existing', description: GIST_DESCRIPTION },
...
describe('writeGistData', () => {
  it('PATCHes the gist with a compressed payload under the current filename', async () => {
