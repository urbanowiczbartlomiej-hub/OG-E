# Changelog archive (releases ≤ 1.9.3)

Older OG-E releases, moved out of the active [`CHANGELOG.md`](../CHANGELOG.md)
to keep it lean — the release script only ever reads the current version's
`## [X.Y.Z]` section, so archiving the rest is safe. Newest archived
version first; full history also lives in git.
